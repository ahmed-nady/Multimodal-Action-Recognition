...
class X3DLateralityPose(nn.Module):
    ...
    def __init__(self,
        ...
        fusion_conv_param = dict(kernel_size=1, padding=0, conv_cfg=conv_cfg, norm_cfg=norm_cfg, act_cfg=act_cfg)
        self.fusion = ConvModule(self.base_channels *3, self.base_channels , **fusion_conv_param)
        self.res_layers = []
    ...
    def _make_stem_layer(self):
        ...
        pooling layer."""
        print("self.conv1_stride",self.conv1_stride)

        self.conv1a_s = ConvModule(
            6,
            self.base_channels,
            kernel_size=(1, 3, 3),
    ...
    def forward(self, x):
        ...
        """
        arms = x[:, 5:11, :, :, :].clone()
        trunk_part1 = x[:, 0:7, :, :, :].clone()
        trunk_part2 = x[:, 11:13, :, :, :].clone()
        torso = torch.cat([trunk_part1, trunk_part2], dim=1)
        legs = x[:, 11:17, :, :, :].clone()

        arms_feats = self.conv1a_s(arms)
        arms_feats = self.conv1a_t(arms_feats)
        torso_feats = self.conv1t_s(torso)
        torso_feats = self.conv1t_t(torso_feats)
        legs_feats = self.conv1l_s(legs)
        legs_feats = self.conv1l_t(legs_feats)
        # x = self.conv1_s(x)
        # x = self.conv1_t(x)
        layer_name = 'layer1t'
        res_layer = getattr(self, layer_name)
        torso_feats = res_layer(torso_feats)
        layer_name = 'layer1a'
        res_layer = getattr(self, layer_name)
        arms_feats = res_layer(arms_feats)
        layer_name = 'layer1l'
        res_layer = getattr(self, layer_name)
        legs_feats = res_layer(legs_feats)

        #====concatenate=====#
        whole_body_features = torch.cat([arms_feats, torso_feats, legs_feats], dim=1)
        x = self.fusion(whole_body_features)

        for layer_name in self.res_layers[3:]:
            res_layer = getattr(self, layer_name)
            x = res_layer(x)
        x = self.conv5(x)
        return x
