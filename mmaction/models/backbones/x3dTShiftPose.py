...
class BlockX3D(nn.Module):
    ...
    # @staticmethod
    def temporalShift(self, x, fold_div=4, inplace=False):
        n, c, t, h, w = x.size()
        fold = c // fold_div
        if inplace:
    ...
    def forward(self, x):
        ...
        def _inner_forward(x):
            ...
            identity = x
            x_shifted = self.temporalShift(x)
...
class X3DTemporalShiftPose(nn.Module):
    ...
    def __init__(self,
        ...
        # ==================================#
        # ==========Nesting STA blocks========#
        # self.sta_block = SpatialTemporalAttention(channels=24,inter_channels=48)
        self.sta_blocks = []
        ...
        for i, num_blocks in enumerate(self.stage_blocks):
            ...
            # ===sta====
            # sta_block = SpatialTemporalAttention(channels=inplanes,inter_channels=48)
            # sta_name = f'sta{i+1}'
            # self.add_module(sta_name,sta_block)
            # self.sta_blocks.append(sta_name)
    ...
    def forward(self, x):
        ...
        # for layer_name,sta_name in zip(self.res_layers,self.sta_blocks):
        for layer_name in self.res_layers:
            res_layer = getattr(self, layer_name)
            x = res_layer(x)
            # sta_block = getattr(self,sta_name)
            # sta_maps = sta_block(x)
            # x = sta_maps*x
...
if __name__ == '__main__':
    ...
    input_data = torch.rand(1, 17, 48, 56, 56)
