...
class C3D(nn.Module):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 fourth_pooling=False,inputPart=None):
        super().__init__()
        ...
        self.fourth_pooling = fourth_pooling
    ...
    def forward(self, x):
        ...
        """
        input = x.clone()
        part_jointsIdices_Dict = {'ra':[6,8,10],'la':[5,7,9],'rl':[12,14,16],'ll':[11,13,15]}
        if self.inputPart:
            #print("self.inputPart")
            part_indecis = part_jointsIdices_Dict[self.inputPart]
            pt1 = x[:,part_indecis[0], :, :, :].clone()
            pt2 = x[:, part_indecis[1], :, :, :].clone()
            pt3 = x[:, part_indecis[2], :, :, :].clone()
            pt1 = pt1.unsqueeze(1)
            pt2 = pt2.unsqueeze(1)
            pt3 = pt3.unsqueeze(1)
            input = torch.cat([pt1, pt2, pt3], dim=1)
