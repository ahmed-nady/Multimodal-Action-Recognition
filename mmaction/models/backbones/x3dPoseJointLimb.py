...
class X3DPoseJointLimb(nn.Module):
    ...
    def __init__(self,
        ...
        self.fusion = ConvModule(self.base_channels *2, self.base_channels , **fusion_conv_param)
        self.res_layers = []
            ...
            if i == 0:
                for input_branch in ['J', 'L']:
                    res_layer = self.make_res_layer(
                        ...
                        **kwargs)
                    self.layer_inplanes = inplanes
                    layer_name = f'layer{i + 1}{input_branch}'
    ...
    def forward(self, x):
        ...
        joint_feats = self.conv1J_s(joints)
        joint_feats = self.conv1J_t(joint_feats)
        limb_feats = self.conv1L_s(limbs)
        limb_feats = self.conv1L_t(limb_feats)
        # x = self.conv1_s(x)
        # x = self.conv1_t(x)
        layer_name = 'layer1J'
        res_layer = getattr(self, layer_name)
        joint_feats = res_layer(joint_feats)
        layer_name = 'layer1L'
        res_layer = getattr(self, layer_name)
        limb_feats = res_layer(limb_feats)

        # ====concatenate=====#
        body_features = torch.cat([joint_feats, limb_feats], dim=1)
        x = self.fusion(body_features)

        for layer_name in self.res_layers[2:]:
            res_layer = getattr(self, layer_name)
            x = res_layer(x)
        x = self.conv5(x)
        return x
