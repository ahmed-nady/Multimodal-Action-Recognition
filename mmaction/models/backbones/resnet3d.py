...
class ResNet3d18(nn.Module):
    ...
    def __init__(self,
        ...
        self.stage_1a = BasicBlock3d(inplanes=inplanes,planes=planes,spatial_stride=1,temporal_stride=1,conv_cfg=conv_cfg,
            norm_cfg=norm_cfg,act_cfg=act_cfg)
        self.stage_1b = BasicBlock3d(inplanes=planes,planes=planes,spatial_stride=1,temporal_stride=1,conv_cfg=conv_cfg,
            norm_cfg=norm_cfg,act_cfg=act_cfg)

        inplanes = planes
        planes = self.base_channels*2
        temporal_stride, spatial_stride =1,2
        downsample = ConvModule(
            ...
            norm_cfg=norm_cfg,
            act_cfg=None)
        self.stage_2a = BasicBlock3d(inplanes=inplanes, planes=planes, spatial_stride=2, temporal_stride=1,downsample=downsample,conv_cfg=conv_cfg,
            norm_cfg=norm_cfg,act_cfg=act_cfg)
        self.stage_2b = BasicBlock3d(inplanes=planes, planes=planes, spatial_stride=1, temporal_stride=1,conv_cfg=conv_cfg,
            norm_cfg=norm_cfg,act_cfg=act_cfg)
        ...
        downsample = ConvModule(
            ...
            norm_cfg=norm_cfg,
            act_cfg=None)
        self.stage_3a = BasicBlock3d(inplanes=inplanes, planes=planes, spatial_stride=2, temporal_stride=1,
                                downsample=downsample,conv_cfg=conv_cfg,norm_cfg=norm_cfg,act_cfg=act_cfg)
        self.stage_3b = BasicBlock3d(inplanes=planes, planes=planes, spatial_stride=1, temporal_stride=1,conv_cfg=conv_cfg,
            norm_cfg=norm_cfg,act_cfg=act_cfg)
        ...
        downsample = ConvModule(
            ...
            norm_cfg=norm_cfg,
            act_cfg=None)
        self.stage_4a = BasicBlock3d(inplanes=inplanes, planes=planes, spatial_stride=2, temporal_stride=1,
                                downsample=downsample,conv_cfg=conv_cfg,norm_cfg=norm_cfg,act_cfg=act_cfg)
        self.stage_4b = BasicBlock3d(inplanes=planes, planes=planes, spatial_stride=1, temporal_stride=1,conv_cfg=conv_cfg,
            norm_cfg=norm_cfg,act_cfg=act_cfg)
...
if __name__ == "__main__":
    ...
    resNet18 = ResNet3d18(in_channels=17,base_channels=32)
    resNet = ResNet3d(depth=50,pretrained=None,
        in_channels=17,
        base_channels=32,
        num_stages=3,
        out_indices=(2, ),
        ...
        dilations=(1, 1, 1))
    # # show output shape
    # print(summary(resNet, resNet, show_input=False))
    # exit(0)
    model_num_params=0
    for parameter in list(resNet.parameters()):
        layer_params =1
        for d in list(parameter.size()):
            layer_params *= d
        model_num_params +=layer_params

    print("model_num_params",model_num_params,model_num_params/1000000)
    input = torch.rand(1,17,48,56,56)
    resNet18(input)

    print(resNet18.parameters())

    # show output shape
    print(summary(resNet18, input, show_input=False))