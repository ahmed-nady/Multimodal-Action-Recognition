...
class GradCamSk:
    ...
    def _calculate_localization_map(self, inputs, use_labels, delta=1e-20):
        ...
        """
        #inputs['imgs'] = inputs['imgs']
        # model forward & backward
        t1 = time.time()
        preds = self.model(gradcam=True, **inputs)
        print('Time of prediction :', time.time() - t1)
        if use_labels:
            labels = inputs['label']
            if labels.ndim == 1:
            ...
            print("using Pred Score:",score)

        t2 = time.time()
        self.model.zero_grad()
        score = torch.sum(score)
        score.backward()
        print('Time of backprob :', time.time() - t2)
        ...
        if self.is_recognizer2d:
            ...
            print("inputs['imgs'].size()",inputs['imgs'].size())
        ...
        return localization_map.squeeze(dim=1), preds.cpu()
    ...
    def _alpha_blending(self, localization_map, input_imgs, alpha):
        ...
        if self.is_recognizer2d:
            ...
            # print("curr_inp",curr_inp)
            curr_inp = input_imgs#curr_inp.permute(0, 2, 3, 4, 1)
    ...
    def __call__(self, inputs,blending_images, use_labels=False, alpha=0.5):
        ...
        blended_imgs = self._alpha_blending(localization_map,blending_images,
                                            alpha)
