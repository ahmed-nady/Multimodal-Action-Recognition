...
from .pose_loading import UniformSampleFrames, PoseDecode
        ...
        for mod in results['modality']:
            ...
            if mod == 'RGB':
                if 'filename' not in results:
                    if self.dataset_name in ['ntu60','ntu120']:
                        results['filename'] = self.data_root + results['frame_dir'] + '_rgb.mp4'
                    else:
                        results['filename'] = self.data_root + results['frame_dir'] + '.mp4'
                # results = self.decordInit(results)
                # results= self.decordDecode(results)
