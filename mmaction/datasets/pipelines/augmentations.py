...
class PoseCompact:
    ...
    def __call__(self, results):
        ...
        #print("crop_quadruple", crop_quadruple)
        return results
...
@PIPELINES.register_module()
class ResizePoseheatmaps:
    def __init__(self,
                 scale,
                 keep_ratio=True,
                 interpolation='bilinear',key='imgs'):
        self.scale = scale
    ...
    def _resize_imgs(self, imgs, new_w, new_h,min_bbox):
        ...
        return [
            mmcv.imresize(
                img[y:y2,x:x2], (new_w, new_h), interpolation=self.interpolation)
            for img in imgs
        ]
    ...
    def __call__(self, results):
        ...
        # results[key] = self._resize_imgs(results[key], new_w, new_h, results['min_bbox'])
        #if 'imgs' in results:
        results[ self.key] = self._resize_imgs(results[ self.key], new_w,new_h,results['min_bbox'])
...
class RandomRescale:
    ...
    def __repr__(self):
        ...
        return repr_str
@PIPELINES.register_module()
class GroupWiseTranslation:
    def __init__(self,max_motion=20):
        self.max_motion = max_motion
    def __call__(self, results):
        """
        Args:
            keypoints: its size is Npersons x num_frames x 17 x 2
        Returns:
        """
        kps =None
        ...
        if self.max_motion>0:
            part_joints_dict ={}
            part_joints_dict['1']=[0,1,2,3,4] #head
            part_joints_dict['2'] = [5,7,9]
            part_joints_dict['3'] = [6,8,10]
            part_joints_dict['4'] =[14,16]
            part_joints_dict['5'] =[13,15]
            #==loop through each part's joints and move them through all frames
            for part_joints_key in part_joints_dict.keys():
                part_joints = part_joints_dict[part_joints_key]
                delta = np.random.randint(1,self.max_motion,(2,))# for dx,dy
                mode = np.random.randint(0,4,(1,))
                ...
                if mode[0]==1:
                    ...
                    dx=-dx
                    dy=-dy
                #Npersons x num_frames x 17 x 2
                for joint_idx in part_joints:
                    #print("keypoints",keypoints.shape)
                    kps[:,:,joint_idx,0]=kps[:,:,joint_idx,0]+ dx
                    kps[:, :, joint_idx, 1] = kps[:, :, joint_idx, 1] + dy
