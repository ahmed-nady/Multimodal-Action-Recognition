...
from ..core import (mean_average_precision, mean_class_accuracy,
                    mmit_mean_average_precision, top_k_accuracy, per_class_accuracy)
...
class BaseDataset(Dataset, metaclass=ABCMeta):
    ...
    def __init__(self,
                 ...
                 power=0,
                 dynamic_length=False,supCont=False):
        super().__init__()

        self.supCont = supCont
    ...
    def save_stats(self,cof_mtx,per_class_acc,save_mode="w"):

        with open('confusion_mtx.csv', save_mode, newline='') as f:
            # create the csv writer
            writer = csv.writer(f)
            writer.writerow(per_class_acc)
            writer.writerows(cof_mtx)
    def evaluate(self,
                 results,
        ...
        allowed_metrics = ['top_k_accuracy', 'mean_class_accuracy', 'mean_average_precision','mmit_mean_average_precision','per_class_accuracy']
        ...
        #results = [item.cpu().numpy() for item in results if not isinstance(item,np.ndarray)]
        np_scores = []
        if type(results) is list:
            for elm in results:
                if isinstance(elm, np.ndarray):
                    np_scores.append(elm)
                else:
                    np_scores.append(elm.cpu().numpy())
            results = np_scores.copy()
        ...
        for metric in metrics:
            ...
            if metric == 'per_class_accuracy':
                per_class_acc,cf_mtx = per_class_accuracy(results, gt_labels)
                print("per_class_accuracy",per_class_acc)
                print("cf_mtx",cf_mtx,per_class_acc)
                self.save_stats(cf_mtx,per_class_acc)
                continue
                eval_results['per_class_accuracy'] = per_class_acc
                #log_msg = f'\nmean_acc\t{mean_acc:.4f}'
                #print_log(log_msg, logger=logger)
                continue
